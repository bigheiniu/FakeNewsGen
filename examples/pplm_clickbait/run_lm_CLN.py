...
class FactDataset(Dataset):
    def __init__(self, tokenizer, args, file_path="train", block_size=512):
        assert os.path.isfile(file_path), "{}".format(file_path)
        directory, filename = os.path.split(file_path)
        cached_features_file = os.path.join(
            directory, args.model_name_or_path + "_cached_lm_" + str(block_size) + "_" + filename
        )
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            #                                                                               pad_to_max_length=True)) for i, j in zip(data_prompt, data_fact)]
            # self.content = [tokenizer.build_inputs_with_special_tokens(tokenizer.encode(i, max_length=self.max_length,
            #                                                                              pad_to_max_length=True)) for i in data_content]
