...
def main():
    ...
    def file_gen():
        ...
        if args.use_fact:
            if args.fact_sep is False:
                prompt_text_list = ["<p-begin> " + i[1] + " <p-end> "+ " <f-begin> "+ " ".join((",".join(i[-2].split("|"))).split(",")) + " <f-end> " + " <c-begin> " for i in prompt_list]

            else:
                prompt_text_list = ["<p-begin> " + i[1] + " <p-end> " + " <c-begin> " for i in prompt_list]
            fact_list = ["<f-begin>" + " ".join((",".join(i[-2].split("|"))).split(",")) + "<f-end>" for i in prompt_list]
            fact_raw = [i[-2] for i in prompt_list]
            ref_content = [i[2] for i in prompt_list]

            index = [i[0] for i in prompt_list]
        else:
            prompt_text_list = [i[1] for i in prompt_list]
            index = list(range(len(prompt_list)))
        ...
        encoded_prompt = [tokenizer.encode(prompt_text, add_special_tokens=False, return_tensors="pt") for prompt_text in
                          prompt_text_list]
        if args.use_fact:
            fact_list = [tokenizer.encode(fact, add_special_tokens=False, return_tensors="pt") for fact in
                          fact_list]
        else:
            fact_list = None
        ...
        with tqdm(total=total) as pbar:
            for i, (e_p, fact)in enumerate(zip(encoded_prompt, fact_list if fact_list else [None] * len(index))):
                ...
                if args.fact_sep:
                    ...
                    output_sequences = model.generate(
                        ...
                        top_p=args.p,
                        repetition_penalty=args.repetition_penalty,
                        condition_label=torch.tensor([1]).to(device=args.device),
                        do_sample=True
                    )
                ...
                fact = tokenizer.decode(fact.cpu()[0].tolist(), clean_up_tokenization_spaces=True)
                # c_label = c_label.cpu()[0].tolist()
                # fout.write("{}\t{}\t{}\t{}\t{}\n".format(text, prompt_text, fact_raw[i], 1))
                # fout.write("{}\t{}\t{}\t{}\t{}\n".format(text, prompt_text, fact_raw[i], 1))
                fout.write("{}\t{}\t{}\t{}\t{}\n".format(prompt_text,fact_raw[i],ref_content[i],text, 1))
                pbar.update(1)
        fout.close()
    def prompt_gen():
        while True:
